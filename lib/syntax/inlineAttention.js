/**
 * A generic micromark extension: paired single-character delimiters that wrap
 * their content in a semantic HTML inline element.
 *
 * This is the engine behind rmmd's custom syntax. Every element in the registry
 * (`=mark=`, `+dfn+`, `~s~`, `^sup^`, ...) is one call to this factory, so they
 * all inherit identical pairing, nesting and flanking behaviour.
 *
 * Adapted from micromark-extension-gfm-strikethrough, generalised over the
 ...
 * @param {string} options.marker - The single delimiter character, e.g. '='.
 ...
 */
export function inlineAttention({ marker, name, maxSize = 2 }) {
