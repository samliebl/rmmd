...
/**
 * Longest term we will match. An abbreviation is a short string by nature, and
 * without a bound a pathological definition builds a pattern the regular
 * expression engine refuses to compile.
 */
const MAX_TERM_LENGTH = 100;

/**
 * Budget for one compiled alternation. Enough terms and even short ones
 * overflow the engine, so terms are matched in batches under this size.
 */
const MAX_PATTERN_LENGTH = 8000;
...
function collectDefinitions(tree) {
  ...
  visit(tree, 'paragraph', (node, index, parent) => {
    ...
    for (const match of matches) {
      ...
      // Over-long terms are not abbreviations; ignore rather than crash.
      if (term && term.length <= MAX_TERM_LENGTH) definitions.set(term, title);
    }
...
function applyDefinitions(tree, definitions) {
  ...
  // Longest first, in batches small enough to compile. Terms applied in an
  // earlier batch are inside an <abbr> by the next pass and are skipped there,
  // so batching preserves longest-match-wins.
  for (const batch of batchTerms(terms)) {
    // `\w` boundaries so `HTML` does not match inside `HTMLElement`.
    const pattern = new RegExp(`(?<!\\w)(${batch.join('|')})(?!\\w)`, 'g');
    applyPattern(tree, pattern, definitions);
  }
}

/** Split escaped terms into groups that each compile safely. */
function batchTerms(terms) {
  const batches = [];
  let batch = [];
  let length = 0;
  ...
  for (const term of terms) {
    ...
    if (batch.length > 0 && length + escaped.length > MAX_PATTERN_LENGTH) {
      batches.push(batch);
      batch = [];
      length = 0;
    }
...
function applyPattern(tree, pattern, definitions) {
